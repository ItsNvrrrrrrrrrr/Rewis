...
struct RewisVM
{
  ...
  Obj* tempRoots[REWIS_MAX_TEMP_ROOTS];
  ...
  // slots by calling rewisEnsureSlots(), a stack is created and this is
  // initialized.
  Value* apiStack;

  RewisConfiguration config;
...
// Creates a new [RewisHandle] for [value].
...
ObjClosure* rewisCompileSource(RewisVM* vm, const char* module,
...
static inline void rewisCallFunction(RewisVM* vm, ObjFiber* fiber,
                                    ObjClosure* closure, int numArgs)
{
  ...
  if (fiber->numFrames + 1 > fiber->frameCapacity)
  {
    ...
    fiber->frames = (CallFrame*)rewisReallocate(vm, fiber->frames,
  ...
  rewisEnsureStack(vm, fiber, needed);
  
  rewisAppendCallFrame(vm, fiber, closure, fiber->stackTop - numArgs);
}
...
//
// Defined here instead of in rewis_value.h because it's critical that this be
// inlined. That means it must be defined in the header, but the rewis_value.h
// header doesn't have a full definitely of RewisVM yet.
static inline ObjClass* rewisGetClassInline(RewisVM* vm, Value value)
{
...
// letter).
static inline bool rewisIsLocalName(const char* name)
{
  return name[0] >= 'a' && name[0] <= 'z';
}

static inline bool rewisIsFalsyValue(Value value)
{
