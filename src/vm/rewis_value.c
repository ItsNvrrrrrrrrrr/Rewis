...
static void initObj(RewisVM* vm, Obj* obj, ObjType type, ObjClass* classObj)
{
  obj->type = type;
  obj->isDark = false;
...
ObjClass* rewisNewSingleClass(RewisVM* vm, int numFields, ObjString* name)
{
  ...
  rewisPushRoot(vm, (Obj*)classObj);
  rewisMethodBufferInit(&classObj->methods);
  rewisPopRoot(vm);

  return classObj;
}

void rewisBindSuperclass(RewisVM* vm, ObjClass* subclass, ObjClass* superclass)
{
  ...
  {
    rewisBindMethod(vm, subclass, i, superclass->methods.data[i]);
  }
}

ObjClass* rewisNewClass(RewisVM* vm, ObjClass* superclass, int numFields,
                       ObjString* name)
{
  // Create the metaclass.
  Value metaclassName = rewisStringFormat(vm, "@ metaclass", OBJ_VAL(name));
  rewisPushRoot(vm, AS_OBJ(metaclassName));

  ObjClass* metaclass = rewisNewSingleClass(vm, 0, AS_STRING(metaclassName));
  metaclass->obj.classObj = vm->classClass;

  rewisPopRoot(vm);

  // Make sure the metaclass isn't collected when we allocate the class.
  rewisPushRoot(vm, (Obj*)metaclass);

  // Metaclasses always inherit Class and do not parallel the non-metaclass
  // hierarchy.
  rewisBindSuperclass(vm, metaclass, vm->classClass);

  ObjClass* classObj = rewisNewSingleClass(vm, numFields, name);
  ...
  rewisBindSuperclass(vm, classObj, superclass);
...
void rewisBindMethod(RewisVM* vm, ObjClass* classObj, int symbol, Method method)
{
  ...
  if (symbol >= classObj->methods.count)
  {
    ...
    rewisMethodBufferFill(vm, &classObj->methods, noMethod,
...
ObjClosure* rewisNewClosure(RewisVM* vm, ObjFn* fn)
{
...
ObjFiber* rewisNewFiber(RewisVM* vm, ObjClosure* closure)
{
  ...
  int stackCapacity = closure == NULL
      ? 1
      : rewisPowerOf2Ceil(closure->fn->maxSlots + 1);
  ...
  if (closure != NULL)
  {
    ...
    rewisAppendCallFrame(vm, fiber, closure, fiber->stack);
...
void rewisEnsureStack(RewisVM* vm, ObjFiber* fiber, int needed)
{
  if (fiber->stackCapacity >= needed) return;
  
  int capacity = rewisPowerOf2Ceil(needed);
  
  Value* oldStack = fiber->stack;
  fiber->stack = (Value*)rewisReallocate(vm, fiber->stack,
...
ObjForeign* rewisNewForeign(RewisVM* vm, ObjClass* classObj, size_t size)
{
...
ObjFn* rewisNewFunction(RewisVM* vm, ObjModule* module, int maxSlots)
{
  FnDebug* debug = ALLOCATE(vm, FnDebug);
  debug->name = NULL;
  rewisIntBufferInit(&debug->sourceLines);

  ObjFn* fn = ALLOCATE(vm, ObjFn);
  initObj(vm, &fn->obj, OBJ_FN, vm->fnClass);
  
  rewisValueBufferInit(&fn->constants);
  rewisByteBufferInit(&fn->code);
  fn->module = module;
...
void rewisFunctionBindName(RewisVM* vm, ObjFn* fn, const char* name, int length)
{
...
Value rewisNewInstance(RewisVM* vm, ObjClass* classObj)
{
...
ObjList* rewisNewList(RewisVM* vm, uint32_t numElements)
{
...
void rewisListInsert(RewisVM* vm, ObjList* list, Value value, uint32_t index)
{
  if (IS_OBJ(value)) rewisPushRoot(vm, AS_OBJ(value));

  // Add a slot at the end of the list.
  rewisValueBufferWrite(vm, &list->elements, NULL_VAL);
...
int rewisListIndexOf(RewisVM* vm, ObjList* list, Value value)
{
  int count = list->elements.count;
  for (int i = 0; i < count; i++)
  {
    Value item = list->elements.data[i];
    if(rewisValuesEqual(item, value)) {
      return i;
    }
  }
  return -1;
...
Value rewisListRemoveAt(RewisVM* vm, ObjList* list, uint32_t index)
{
  Value removed = list->elements.data[index];

  if (IS_OBJ(removed)) rewisPushRoot(vm, AS_OBJ(removed));
  ...
  {
    list->elements.data = (Value*)rewisReallocate(vm, list->elements.data,
        sizeof(Value) * list->elements.capacity,
        sizeof(Value) * (list->elements.capacity / GROW_FACTOR));
    list->elements.capacity /= GROW_FACTOR;
  }

  if (IS_OBJ(removed)) rewisPopRoot(vm);

  list->elements.count--;
  return removed;
}

ObjMap* rewisNewMap(RewisVM* vm)
{
...
static inline uint32_t hashNumber(double num)
{
  ...
  return hashBits(rewisDoubleToBits(num));
}
                      ...
                      MapEntry** result)
{
  ...
  do
  {
    ...
    else if (rewisValuesEqual(entry->key, key))
    {
      // We found the key.
      *result = entry;
      return true;
...
static void resizeMap(RewisVM* vm, ObjMap* map, uint32_t capacity)
{
...
Value rewisMapGet(ObjMap* map, Value key)
{
  MapEntry* entry;
...
void rewisMapSet(RewisVM* vm, ObjMap* map, Value key, Value value)
{
...
void rewisMapClear(RewisVM* vm, ObjMap* map)
{
  DEALLOCATE(vm, map->entries);
  map->entries = NULL;
  map->capacity = 0;
...
Value rewisMapRemoveKey(RewisVM* vm, ObjMap* map, Value key)
{
  MapEntry* entry;
...
ObjModule* rewisNewModule(RewisVM* vm, ObjString* name)
{
  ...
  rewisSymbolTableInit(&module->variableNames);
  rewisValueBufferInit(&module->variables);
...
Value rewisNewRange(RewisVM* vm, double from, double to, bool isInclusive)
{
...
// hash.
static ObjString* allocateString(RewisVM* vm, size_t length)
{
...
Value rewisNewString(RewisVM* vm, const char* text)
{
  return rewisNewStringLength(vm, text, strlen(text));
}

Value rewisNewStringLength(RewisVM* vm, const char* text, size_t length)
{
...
Value rewisNewStringFromRange(RewisVM* vm, ObjString* source, int start,
                             uint32_t count, int step)
{
  ...
  {
    length += rewisUtf8DecodeNumBytes(from[start + i * step]);
  }
  ...
  for (uint32_t i = 0; i < count; i++)
  {
    ...
    int codePoint = rewisUtf8Decode(from + index, source->length - index);

    if (codePoint != -1)
    {
      to += rewisUtf8Encode(codePoint, to);
    }
  }
...
Value rewisNumToString(RewisVM* vm, double value)
{
  ...
  return rewisNewStringLength(vm, buffer, length);
}

Value rewisStringFromCodePoint(RewisVM* vm, int value)
{
  int length = rewisUtf8EncodeNumBytes(value);
  ASSERT(length != 0, "Value out of range.");

  ObjString* string = allocateString(vm, length);

  rewisUtf8Encode(value, (uint8_t*)string->value);
  hashString(string);

  return OBJ_VAL(string);
}

Value rewisStringFromByte(RewisVM *vm, uint8_t value)
{
  int length = 1;
  ObjString* string = allocateString(vm, length);
...
Value rewisStringFormat(RewisVM* vm, const char* format, ...)
{
  va_list argList;
...
Value rewisStringCodePointAt(RewisVM* vm, ObjString* string, uint32_t index)
{
  ASSERT(index < string->length, "Index out of bounds.");

  int codePoint = rewisUtf8Decode((uint8_t*)string->value + index,
                                 string->length - index);
  if (codePoint == -1)
  {
    ...
    bytes[1] = '\0';
    return rewisNewStringLength(vm, bytes, 1);
  }

  return rewisStringFromCodePoint(vm, codePoint);
}

// Uses the Boyer-Moore-Horspool string matching algorithm.
uint32_t rewisStringFind(ObjString* haystack, ObjString* needle, uint32_t start)
{
...
ObjUpvalue* rewisNewUpvalue(RewisVM* vm, Value* value)
{
...
void rewisGrayObj(RewisVM* vm, Obj* obj)
{
  if (obj == NULL) return;
...
void rewisGrayValue(RewisVM* vm, Value value)
{
  if (!IS_OBJ(value)) return;
  rewisGrayObj(vm, AS_OBJ(value));
}

void rewisGrayBuffer(RewisVM* vm, ValueBuffer* buffer)
{
  for (int i = 0; i < buffer->count; i++)
  {
    rewisGrayValue(vm, buffer->data[i]);
  }
}

static void blackenClass(RewisVM* vm, ObjClass* classObj)
{
  // The metaclass.
  rewisGrayObj(vm, (Obj*)classObj->obj.classObj);

  // The superclass.
  rewisGrayObj(vm, (Obj*)classObj->superclass);
  ...
  for (int i = 0; i < classObj->methods.count; i++)
  {
    ...
    {
      rewisGrayObj(vm, (Obj*)classObj->methods.data[i].as.closure);
    }
  }

  rewisGrayObj(vm, (Obj*)classObj->name);

  if(!IS_NULL(classObj->attributes)) rewisGrayObj(vm, AS_OBJ(classObj->attributes));
...
static void blackenClosure(RewisVM* vm, ObjClosure* closure)
{
  // Mark the function.
  rewisGrayObj(vm, (Obj*)closure->fn);

  // Mark the upvalues.
  for (int i = 0; i < closure->fn->numUpvalues; i++)
  {
    rewisGrayObj(vm, (Obj*)closure->upvalues[i]);
  }
...
static void blackenFiber(RewisVM* vm, ObjFiber* fiber)
{
  // Stack functions.
  for (int i = 0; i < fiber->numFrames; i++)
  {
    rewisGrayObj(vm, (Obj*)fiber->frames[i].closure);
  }

  // Stack variables.
  for (Value* slot = fiber->stack; slot < fiber->stackTop; slot++)
  {
    rewisGrayValue(vm, *slot);
  }
  ...
  while (upvalue != NULL)
  {
    rewisGrayObj(vm, (Obj*)upvalue);
    upvalue = upvalue->next;
  }

  // The caller.
  rewisGrayObj(vm, (Obj*)fiber->caller);
  rewisGrayValue(vm, fiber->error);
...
static void blackenFn(RewisVM* vm, ObjFn* fn)
{
  // Mark the constants.
  rewisGrayBuffer(vm, &fn->constants);
...
static void blackenForeign(RewisVM* vm, ObjForeign* foreign)
{
...
static void blackenInstance(RewisVM* vm, ObjInstance* instance)
{
  rewisGrayObj(vm, (Obj*)instance->obj.classObj);

  // Mark the fields.
  for (int i = 0; i < instance->obj.classObj->numFields; i++)
  {
    rewisGrayValue(vm, instance->fields[i]);
  }
...
static void blackenList(RewisVM* vm, ObjList* list)
{
  // Mark the elements.
  rewisGrayBuffer(vm, &list->elements);
...
static void blackenMap(RewisVM* vm, ObjMap* map)
{
  // Mark the entries.
  for (uint32_t i = 0; i < map->capacity; i++)
  {
    ...
    rewisGrayValue(vm, entry->key);
    rewisGrayValue(vm, entry->value);
  }
...
static void blackenModule(RewisVM* vm, ObjModule* module)
{
  // Top-level variables.
  for (int i = 0; i < module->variables.count; i++)
  {
    rewisGrayValue(vm, module->variables.data[i]);
  }

  rewisBlackenSymbolTable(vm, &module->variableNames);

  rewisGrayObj(vm, (Obj*)module->name);

  // Keep track of how much memory is still in use.
  vm->bytesAllocated += sizeof(ObjModule);
}

static void blackenRange(RewisVM* vm, ObjRange* range)
{
  // Keep track of how much memory is still in use.
  vm->bytesAllocated += sizeof(ObjRange);
}

static void blackenString(RewisVM* vm, ObjString* string)
{
  // Keep track of how much memory is still in use.
  vm->bytesAllocated += sizeof(ObjString) + string->length + 1;
}

static void blackenUpvalue(RewisVM* vm, ObjUpvalue* upvalue)
{
  // Mark the closed-over object (in case it is closed).
  rewisGrayValue(vm, upvalue->closed);

  // Keep track of how much memory is still in use.
  vm->bytesAllocated += sizeof(ObjUpvalue);
}

static void blackenObject(RewisVM* vm, Obj* obj)
{
#if REWIS_DEBUG_TRACE_MEMORY
  printf("mark ");
  rewisDumpValue(OBJ_VAL(obj));
  printf(" @ %p\n", obj);
#endif
...
void rewisBlackenObjects(RewisVM* vm)
{
...
void rewisFreeObj(RewisVM* vm, Obj* obj)
{
#if REWIS_DEBUG_TRACE_MEMORY
  printf("free ");
  rewisDumpValue(OBJ_VAL(obj));
  ...
  {
    case OBJ_CLASS:
      rewisMethodBufferClear(vm, &((ObjClass*)obj)->methods);
      break;
    ...
    {
      ObjFn* fn = (ObjFn*)obj;
      rewisValueBufferClear(vm, &fn->constants);
      rewisByteBufferClear(vm, &fn->code);
      rewisIntBufferClear(vm, &fn->debug->sourceLines);
    ...
    case OBJ_FOREIGN:
      rewisFinalizeForeign(vm, (ObjForeign*)obj);
      break;

    case OBJ_LIST:
      rewisValueBufferClear(vm, &((ObjList*)obj)->elements);
      break;
    ...
    case OBJ_MODULE:
      rewisSymbolTableClear(vm, &((ObjModule*)obj)->variableNames);
      rewisValueBufferClear(vm, &((ObjModule*)obj)->variables);
      break;
...
ObjClass* rewisGetClass(RewisVM* vm, Value value)
{
  return rewisGetClassInline(vm, value);
}

bool rewisValuesEqual(Value a, Value b)
{
  if (rewisValuesSame(a, b)) return true;
  ...
  switch (aObj->type)
  {
    ...
    case OBJ_STRING:
    {
      ...
      rewisStringEqualsCString(aString, bString->value, bString->length);
    }
