...
#if REWIS_DEBUG_DUMP_COMPILED_CODE
                       ...
                       const char* format, va_list args)
{
  ...
  // Only report errors if there is a RewisErrorFn to handle them.
  ...
  parser->vm->config.errorFn(parser->vm, REWIS_ERROR_COMPILE,
...
static int addConstant(Compiler* compiler, Value constant)
{
  ...
  {
    Value existing = rewisMapGet(compiler->constants, constant);
  ...
  {
    if (IS_OBJ(constant)) rewisPushRoot(compiler->parser->vm, AS_OBJ(constant));
    rewisValueBufferWrite(compiler->parser->vm, &compiler->fn->constants,
                         constant);
    if (IS_OBJ(constant)) rewisPopRoot(compiler->parser->vm);
    
    if (compiler->constants == NULL)
    {
      compiler->constants = rewisNewMap(compiler->parser->vm);
    }
    rewisMapSet(compiler->parser->vm, compiler->constants, constant,
                         ...
                         bool isMethod)
{
  ...
  compiler->attributes = rewisNewMap(parser->vm);
  compiler->fn = rewisNewFunction(parser->vm, parser->module,
                                 compiler->numLocals);
}
...
{
  ByteBuffer string;
  rewisByteBufferInit(&string);
  rewisByteBufferWrite(parser->vm, &string, firstChar);

  while (isName(peekChar(parser)) || isDigit(peekChar(parser)))
  {
    char c = nextChar(parser);
    rewisByteBufferWrite(parser->vm, &string, c);
  }
  ...
  parser->next.value = rewisNewStringLength(parser->vm,
                                            (char*)string.data, string.count);

  rewisByteBufferClear(parser->vm, &string);
...
static void readUnicodeEscape(Parser* parser, ByteBuffer* string, int length)
{
  ...
  int numBytes = rewisUtf8EncodeNumBytes(value);
  if (numBytes != 0)
  {
    rewisByteBufferFill(parser->vm, string, 0, numBytes);
    rewisUtf8Encode(value, string->data + string->count - numBytes);
  }
}
...
static void readString(Parser* parser)
{
  ...
  for (;;)
  {
    ...
    if (c == '\\')
    {
      ...
      {
        case '"':  rewisByteBufferWrite(parser->vm, &string, '"'); break;
        case '\\': rewisByteBufferWrite(parser->vm, &string, '\\'); break;
        case '%':  rewisByteBufferWrite(parser->vm, &string, '%'); break;
        case '0':  rewisByteBufferWrite(parser->vm, &string, '\0'); break;
        case 'a':  rewisByteBufferWrite(parser->vm, &string, '\a'); break;
        case 'b':  rewisByteBufferWrite(parser->vm, &string, '\b'); break;
        case 'e':  rewisByteBufferWrite(parser->vm, &string, '\33'); break;
        case 'f':  rewisByteBufferWrite(parser->vm, &string, '\f'); break;
        case 'n':  rewisByteBufferWrite(parser->vm, &string, '\n'); break;
        case 'r':  rewisByteBufferWrite(parser->vm, &string, '\r'); break;
        case 't':  rewisByteBufferWrite(parser->vm, &string, '\t'); break;
        case 'u':  readUnicodeEscape(parser, &string, 4); break;
        case 'U':  readUnicodeEscape(parser, &string, 8); break;
        case 'v':  rewisByteBufferWrite(parser->vm, &string, '\v'); break;
        case 'x':
          rewisByteBufferWrite(parser->vm, &string,
      ...
      default:
        ...
        else
        {
          ...
          else
          {
            ...
            // meaningful code units in Rewis, the lexer works on raw bytes,
...
{
  rewisByteBufferWrite(compiler->parser->vm, &compiler->fn->code, (uint8_t)byte);
  
  // Assume the instruction is associated with the most recently consumed token.
  rewisIntBufferWrite(compiler->parser->vm, &compiler->fn->debug->sourceLines,
...
static int declareVariable(Compiler* compiler, Token* token)
{
  ...
  {
    int line = -1;
    int symbol = rewisDefineVariable(compiler->parser->vm,
...
static Variable resolveName(Compiler* compiler, const char* name, int length)
{
  ...
  variable.index = rewisSymbolTableFind(&compiler->parser->module->variableNames,
                                       name, length);
  return variable;
}
                          ...
                          const char* debugName, int debugNameLength)
{
  ...
  rewisFunctionBindName(compiler->parser->vm, compiler->fn,
  ...
  #if REWIS_DEBUG_DUMP_COMPILED_CODE
    rewisDumpCode(compiler->parser->vm, compiler->fn);
  #endif
...
{
  return rewisSymbolTableEnsure(compiler->parser->vm,
...
{
  int symbol = rewisSymbolTableFind(&compiler->parser->module->variableNames,
                                   name, strlen(name));
...
static void field(Compiler* compiler, bool canAssign)
{
  ...
  else
  {
    ...
    field = rewisSymbolTableEnsure(compiler->parser->vm, &enclosingClass->fields,
...
static void name(Compiler* compiler, bool canAssign)
{
  ...
  // on this.
  if (rewisIsLocalName(token->start) && getEnclosingClass(compiler) != NULL)
  {
    loadThis(compiler);
  ...
  if (variable.index == -1)
  {
    ...
    variable.index = rewisDeclareVariable(compiler->parser->vm,
                                         compiler->parser->module,
                                         token->start, token->length,
                                         token->line);
...
static void forStatement(Compiler* compiler)
{
  ...
  // actually get names that aren't valid Rewis identfiers, but that's the basic
  // idea.
  //
...
//
// Construction is a two-stage process in Rewis that involves two separate
                         ...
                         const char* name, int length)
{
  ...
  rewisIntBufferWrite(compiler->parser->vm, methods, symbol);
  return symbol;
}
...
static bool method(Compiler* compiler, Variable classVariable)
{
  ...
  if (isForeign)
  {
    ...
    emitConstant(compiler, rewisNewStringLength(compiler->parser->vm,
...
static void classDefinition(Compiler* compiler, bool isForeign)
{
  ...
  Value classNameString = rewisNewStringLength(compiler->parser->vm,
  ...
  classInfo.classAttributes = compiler->attributes->count > 0 
        ? rewisNewMap(compiler->parser->vm) 
        : NULL;
  ...
  // bytecode will be adjusted by [rewisBindMethod] to take inherited fields
  // into account.
  rewisSymbolTableInit(&classInfo.fields);
  
  // Set up symbol buffers to track duplicate static and instance methods.
  rewisIntBufferInit(&classInfo.methods);
  rewisIntBufferInit(&classInfo.staticMethods);
  ...
  rewisSymbolTableClear(compiler->parser->vm, &classInfo.fields);
  rewisIntBufferClear(compiler->parser->vm, &classInfo.methods);
  rewisIntBufferClear(compiler->parser->vm, &classInfo.staticMethods);
  compiler->enclosingClass = NULL;
  popScope(compiler);
}
...
static void import(Compiler* compiler)
{
  ...
  do
  {
    ...
    int sourceVariableConstant = addConstant(compiler,
          rewisNewStringLength(compiler->parser->vm,
...
ObjFn* rewisCompile(RewisVM* vm, ObjModule* module, const char* source,
...
void rewisBindMethodCode(ObjClass* classObj, ObjFn* fn)
{
  int ip = 0;
  for (;;)
  {
    ...
    switch (instruction)
    {
      ...
      case CODE_CLOSURE:
      {
        ...
        rewisBindMethodCode(classObj, AS_FN(fn->constants.data[constant]));
        break;
      }
...
void rewisMarkCompiler(RewisVM* vm, Compiler* compiler)
{
  rewisGrayValue(vm, compiler->parser->current.value);
  rewisGrayValue(vm, compiler->parser->previous.value);
  rewisGrayValue(vm, compiler->parser->next.value);

  // Walk up the parent chain to mark the outer compilers too. The VM only
  // tracks the innermost one.
  do
  {
    rewisGrayObj(vm, (Obj*)compiler->fn);
    rewisGrayObj(vm, (Obj*)compiler->constants);
    rewisGrayObj(vm, (Obj*)compiler->attributes);
    
    if (compiler->enclosingClass != NULL)
    {
      rewisBlackenSymbolTable(vm, &compiler->enclosingClass->fields);

      if(compiler->enclosingClass->methodAttributes != NULL) 
      {
        rewisGrayObj(vm, (Obj*)compiler->enclosingClass->methodAttributes);
      }
      if(compiler->enclosingClass->classAttributes != NULL) 
      {
        rewisGrayObj(vm, (Obj*)compiler->enclosingClass->classAttributes);
      }
    }
...
static void disallowAttributes(Compiler* compiler)
{
  if (compiler->numAttributes > 0)
  {
    ...
    rewisMapClear(compiler->parser->vm, compiler->attributes);
    compiler->numAttributes = 0;
  }
}
...
{
  RewisVM* vm = compiler->parser->vm;

  if(IS_OBJ(group)) rewisPushRoot(vm, AS_OBJ(group));
  if(IS_OBJ(key))   rewisPushRoot(vm, AS_OBJ(key));
  if(IS_OBJ(value)) rewisPushRoot(vm, AS_OBJ(value));

  Value groupMapValue = rewisMapGet(compiler->attributes, group);
  if(IS_UNDEFINED(groupMapValue)) 
  {
    groupMapValue = OBJ_VAL(rewisNewMap(vm));
    rewisMapSet(vm, compiler->attributes, group, groupMapValue);
  }
  ...
  Value keyItemsValue = rewisMapGet(groupMap, key);
  if(IS_UNDEFINED(keyItemsValue)) 
  {
    keyItemsValue = OBJ_VAL(rewisNewList(vm, 0));
    rewisMapSet(vm, groupMap, key, keyItemsValue);
  }

  //keyItems.add(value)
  ObjList* keyItems = AS_LIST(keyItemsValue);
  rewisValueBufferWrite(vm, &keyItems->elements, value);

  if(IS_OBJ(group)) rewisPopRoot(vm);
  if(IS_OBJ(key))   rewisPopRoot(vm);
...
static void copyAttributes(Compiler* compiler, ObjMap* into)
{
  ...
  for(uint32_t attrIdx = 0; attrIdx < compiler->attributes->capacity; attrIdx++)
  {
    ...
    rewisMapSet(vm, into, attrEntry->key, attrEntry->value);
  }
  
  rewisMapClear(vm, compiler->attributes);
}
            ...
            bool isStatic, const char* fullSignature, int32_t length) 
{
  ...
  ObjMap* methodAttr = rewisNewMap(vm);
  rewisPushRoot(vm, (Obj*)methodAttr);
  ...
  if(compiler->enclosingClass->methodAttributes == NULL) {
    compiler->enclosingClass->methodAttributes = rewisNewMap(vm);
  }
  
  // Store the method attributes in the class map
  Value key = rewisNewStringLength(vm, fullSignatureWithPrefix, fullLength);
  rewisMapSet(vm, compiler->enclosingClass->methodAttributes, key, OBJ_VAL(methodAttr));
