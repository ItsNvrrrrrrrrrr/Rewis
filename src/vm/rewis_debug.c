...
void rewisDebugPrintStackTrace(RewisVM* vm)
{
  ...
  {
    vm->config.errorFn(vm, REWIS_ERROR_RUNTIME,
  ...
  for (int i = fiber->numFrames - 1; i >= 0; i--)
  {
    ...
    // detail of what part of the core module is written in C and what is Rewis.
    if (fn->module->name == NULL) continue;
    
    // -1 because IP has advanced past the instruction that it just executed.
    int line = fn->debug->sourceLines.data[frame->ip - fn->code.data - 1];
    vm->config.errorFn(vm, REWIS_ERROR_STACK_TRACE,
...
static int dumpInstruction(RewisVM* vm, ObjFn* fn, int i, int* lastLine)
{
  int start = i;
  ...
  switch (code)
  {
    case CODE_CONSTANT:
    {
      ...
      rewisDumpValue(fn->constants.data[constant]);
      printf("'\n");
      break;
    }
    ...
    case CODE_IMPORT_MODULE:
    {
      ...
      rewisDumpValue(fn->constants.data[name]);
      printf("'\n");
      break;
    }
    ...
    case CODE_IMPORT_VARIABLE:
    {
      ...
      rewisDumpValue(fn->constants.data[variable]);
      printf("'\n");
      break;
    }
