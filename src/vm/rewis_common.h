#ifndef rewis_common_h
#define rewis_common_h

// This header contains macros and defines used across the entire Rewis
// implementation. In particular, it contains "configuration" defines that
// control how Rewis works. Some of these are only used while hacking on Rewis
// itself.
//
// This header is *not* intended to be included by code outside of Rewis itself.

// Rewis pervasively uses the C99 integer types (uint16_t, etc.) along with some
...
// If true, then Rewis uses a NaN-tagged double for its core value
...
//
// Defaults to on.
#ifndef REWIS_NAN_TAGGING
  #define REWIS_NAN_TAGGING 1
#endif
...
#ifndef REWIS_COMPUTED_GOTO
  #if defined(_MSC_VER) && !defined(__clang__)
    // No computed gotos in Visual Studio.
    #define REWIS_COMPUTED_GOTO 0
  #else
    #define REWIS_COMPUTED_GOTO 1
  #endif
#endif
...
// corresponding `REWIS_OPT_<name>` define to `0`.
#ifndef REWIS_OPT_META
  #define REWIS_OPT_META 1
#endif

#ifndef REWIS_OPT_RANDOM
  #define REWIS_OPT_RANDOM 1
#endif

// These flags are useful for debugging and hacking on Rewis itself. They are not
...
#define ALLOCATE(vm, type)                                                     \
    ((type*)rewisReallocate(vm, NULL, 0, sizeof(type)))
...
// assertion fails, there is a bug in Rewis.
//
