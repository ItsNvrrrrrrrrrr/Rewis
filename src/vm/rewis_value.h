#ifndef rewis_value_h
#define rewis_value_h

#include <stdbool.h>
#include <string.h>

#include "rewis_common.h"
#include "rewis_math.h"
#include "rewis_utils.h"

// This defines the built-in types and their core representations in memory.
// Since Rewis is dynamically typed, any variable can hold a value of any type,
...
// storage location that can hold any Rewis value. The stack, module variables,
...
// Rewis implementation calls these "Obj", or objects, though to a user, all
// values are objects.
//
...
//
// The representation is controlled by the `REWIS_NAN_TAGGING` define. If that's
...
#define AS_NUM(value)       (rewisValueToNum(value))             // double
...
// These macros promote a primitive C value to a full Rewis Value. There are
...
#define NUM_VAL(num) (rewisNumToValue(num))                      // double
#define OBJ_VAL(obj) (rewisObjectToValue((Obj*)(obj)))           // Any Obj___*

// These perform type tests on a Value, returning `true` if the Value is of the
// given type.
#define IS_BOOL(value) (rewisIsBool(value))                      // Bool
#define IS_CLASS(value) (rewisIsObjType(value, OBJ_CLASS))       // ObjClass
#define IS_CLOSURE(value) (rewisIsObjType(value, OBJ_CLOSURE))   // ObjClosure
#define IS_FIBER(value) (rewisIsObjType(value, OBJ_FIBER))       // ObjFiber
#define IS_FN(value) (rewisIsObjType(value, OBJ_FN))             // ObjFn
#define IS_FOREIGN(value) (rewisIsObjType(value, OBJ_FOREIGN))   // ObjForeign
#define IS_INSTANCE(value) (rewisIsObjType(value, OBJ_INSTANCE)) // ObjInstance
#define IS_LIST(value) (rewisIsObjType(value, OBJ_LIST))         // ObjList
#define IS_MAP(value) (rewisIsObjType(value, OBJ_MAP))           // ObjMap
#define IS_RANGE(value) (rewisIsObjType(value, OBJ_RANGE))       // ObjRange
#define IS_STRING(value) (rewisIsObjType(value, OBJ_STRING))     // ObjString
...
#define CONST_STRING(vm, text) rewisNewStringLength((vm), (text), sizeof(text) - 1)
...
typedef struct sObjUpvalue
{
  ...
  // collected, but they are not first class Rewis objects.
  Obj obj;
...
typedef bool (*Primitive)(RewisVM* vm, Value* args);
...
typedef enum
{
  ...
  // initial fiber used by a call to `rewisCall()` or `rewisInterpret()`.
  FIBER_ROOT,
...
// [stackStart].
static inline void rewisAppendCallFrame(RewisVM* vm, ObjFiber* fiber,
...
static inline bool rewisHasError(const ObjFiber* fiber)
{
...
static inline bool rewisMapIsValidKey(Value arg);
...
// [needle].
uint32_t rewisStringFind(ObjString* haystack, ObjString* needle,
                        uint32_t startIndex);

// Returns true if [a] and [b] represent the same string.
static inline bool rewisStringEqualsCString(const ObjString* a,
...
//
// Unlike rewisGetClassInline in rewis_vm.h, this is not inlined. Inlining helps
...
static inline bool rewisValuesSame(Value a, Value b)
{
#if REWIS_NAN_TAGGING
...
// [IS_BOOL].
static inline bool rewisIsBool(Value value)
{
#if REWIS_NAN_TAGGING
...
static inline bool rewisIsObjType(Value value, ObjType type)
{
  return IS_OBJ(value) && AS_OBJ(value)->type == type;
}

// Converts the raw object pointer [obj] to a [Value].
static inline Value rewisObjectToValue(Obj* obj)
{
#if REWIS_NAN_TAGGING
...
static inline double rewisValueToNum(Value value)
{
#if REWIS_NAN_TAGGING
  return rewisDoubleFromBits(value);
#else
  return value.as.num;
#endif
}
...
static inline Value rewisNumToValue(double num)
{
#if REWIS_NAN_TAGGING
  return rewisDoubleToBits(num);
#else
  Value value;
  value.type = VAL_NUM;
  value.as.num = num;
