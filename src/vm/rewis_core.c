...
DEF_PRIMITIVE(fiber_new)
{
  ...
  RETURN_OBJ(rewisNewFiber(vm, closure));
}
...
static bool runFiber(RewisVM* vm, ObjFiber* fiber, Value* args, bool isCall,
                     bool hasValue, const char* verb)
{

  if (rewisHasError(fiber))
  {
...
DEF_PRIMITIVE(fiber_isDone)
{
  ...
  RETURN_BOOL(runFiber->numFrames == 0 || rewisHasError(runFiber));
}
...
DEF_PRIMITIVE(fiber_try)
{
  ...
  if (!rewisHasError(vm->fiber)) vm->fiber->state = FIBER_TRY;
  return false;
}
...
static void call_fn(RewisVM* vm, Value* args, int numArgs)
{
  // +1 to include the function itself.
  rewisCallFunction(vm, vm->fiber, AS_CLOSURE(args[0]), numArgs + 1);
}
...
DEF_PRIMITIVE(list_filled)
{
  ...
  ObjList* list = rewisNewList(vm, size);
...
{
  RETURN_OBJ(rewisNewList(vm, 0));
}

DEF_PRIMITIVE(list_add)
{
  rewisValueBufferWrite(vm, &AS_LIST(args[0])->elements, args[1]);
  RETURN_VAL(args[1]);
}
...
{
  rewisValueBufferClear(vm, &AS_LIST(args[0])->elements);
  RETURN_NULL;
}
...
DEF_PRIMITIVE(list_insert)
{
  ...
  rewisListInsert(vm, list, args[2], index);
  RETURN_VAL(args[2]);
}
...
DEF_PRIMITIVE(list_removeAt)
{
  ...
  RETURN_VAL(rewisListRemoveAt(vm, list, index));
}

DEF_PRIMITIVE(list_removeValue) {
  ObjList* list = AS_LIST(args[0]);
  int index = rewisListIndexOf(vm, list, args[1]);
...
{
  ObjList* list = AS_LIST(args[0]);
  RETURN_NUM(rewisListIndexOf(vm, list, args[1]));
}
...
DEF_PRIMITIVE(list_subscript)
{
  ...
  ObjList* result = rewisNewList(vm, count);
...
DEF_PRIMITIVE(map_subscript)
{
  ...
  Value value = rewisMapGet(map, args[1]);
...
DEF_PRIMITIVE(map_subscriptSetter)
{
  ...
  rewisMapSet(vm, AS_MAP(args[0]), args[1], args[2]);
  RETURN_VAL(args[2]);
}
...
{
  rewisMapClear(vm, AS_MAP(args[0]));
  RETURN_NULL;
}
...
DEF_PRIMITIVE(map_containsKey)
{
  ...
  RETURN_BOOL(!IS_UNDEFINED(rewisMapGet(AS_MAP(args[0]), args[1])));
}
...
DEF_PRIMITIVE(map_remove)
{
  ...
  RETURN_VAL(rewisMapRemoveKey(vm, AS_MAP(args[0]), args[1]));
}
...
DEF_NUM_CONSTANT(nan,      REWIS_DOUBLE_NAN)
...
DEF_PRIMITIVE(num_dotDot)
{
  ...
  RETURN_VAL(rewisNewRange(vm, from, to, true));
}
...
DEF_PRIMITIVE(num_dotDotDot)
{
  ...
  double from = AS_NUM(args[0]);
  double to = AS_NUM(args[1]);
  RETURN_VAL(rewisNewRange(vm, from, to, false));
}
...
{
  RETURN_VAL(rewisNumToString(vm, AS_NUM(args[0])));
}
...
{
  RETURN_BOOL(rewisValuesEqual(args[1], args[2]));
}
...
{
  RETURN_BOOL(rewisValuesEqual(args[0], args[1]));
}
...
DEF_PRIMITIVE(object_is)
{
  ...
  ObjClass *classObj = rewisGetClass(vm, args[0]);
...
DEF_PRIMITIVE(object_toString)
{
  ...
  RETURN_VAL(rewisStringFormat(vm, "instance of @", name));
}

DEF_PRIMITIVE(object_type)
{
  RETURN_OBJ(rewisGetClass(vm, args[0]));
}
...
DEF_PRIMITIVE(range_toString)
{
  ...
  Value from = rewisNumToString(vm, range->from);
  rewisPushRoot(vm, AS_OBJ(from));

  Value to = rewisNumToString(vm, range->to);
  rewisPushRoot(vm, AS_OBJ(to));

  Value result = rewisStringFormat(vm, "@$@", from,
...
DEF_PRIMITIVE(string_fromCodePoint)
{
  ...
  RETURN_VAL(rewisStringFromCodePoint(vm, codePoint));
}
...
DEF_PRIMITIVE(string_fromByte)
{
  ...
  RETURN_VAL(rewisStringFromByte(vm, (uint8_t) byte));
}
...
DEF_PRIMITIVE(string_codePointAt)
{
  ...
  RETURN_NUM(rewisUtf8Decode((uint8_t*)string->value + index,
                            string->length - index));
}
...
DEF_PRIMITIVE(string_contains)
{
  ...
  RETURN_BOOL(rewisStringFind(string, search, 0) != UINT32_MAX);
}
...
DEF_PRIMITIVE(string_indexOf1)
{
  ...
  uint32_t index = rewisStringFind(string, search, 0);
...
DEF_PRIMITIVE(string_indexOf2)
{
  ...
  uint32_t index = rewisStringFind(string, search, start);
  RETURN_NUM(index == UINT32_MAX ? -1 : (int)index);
}
...
DEF_PRIMITIVE(string_iteratorValue)
{
  ...
  RETURN_VAL(rewisStringCodePointAt(vm, string, index));
}
...
DEF_PRIMITIVE(string_plus)
{
  ...
  RETURN_VAL(rewisStringFormat(vm, "@@", args[0], args[1]));
}
...
DEF_PRIMITIVE(string_subscript)
{
  ...
  RETURN_VAL(rewisNewStringFromRange(vm, string, start, count, step));
}
...
static ObjClass* defineClass(RewisVM* vm, ObjModule* module, const char* name)
{
  ObjString* nameString = AS_STRING(rewisNewString(vm, name));
  rewisPushRoot(vm, (Obj*)nameString);

  ObjClass* classObj = rewisNewSingleClass(vm, 0, nameString);

  rewisDefineVariable(vm, module, name, nameString->length, OBJ_VAL(classObj), NULL);
...
{
  ObjModule* coreModule = rewisNewModule(vm, NULL);
  rewisPushRoot(vm, (Obj*)coreModule);
  
  // The core module's key is null in the module map.
  rewisMapSet(vm, vm->modules, NULL_VAL, OBJ_VAL(coreModule));
  rewisPopRoot(vm); // coreModule.
  ...
  rewisBindSuperclass(vm, vm->classClass, vm->objectClass);
  ...
  // collected.
  rewisBindSuperclass(vm, objectMetaclass, vm->classClass);
  ...
  rewisInterpret(vm, NULL, coreModuleSource);

  vm->boolClass = AS_CLASS(rewisFindVariable(vm, coreModule, "Bool"));
  PRIMITIVE(vm->boolClass, "toString", bool_toString);
  PRIMITIVE(vm->boolClass, "!", bool_not);

  vm->fiberClass = AS_CLASS(rewisFindVariable(vm, coreModule, "Fiber"));
  ...
  vm->fnClass = AS_CLASS(rewisFindVariable(vm, coreModule, "Fn"));
  ...
  vm->nullClass = AS_CLASS(rewisFindVariable(vm, coreModule, "Null"));
  PRIMITIVE(vm->nullClass, "!", null_not);
  PRIMITIVE(vm->nullClass, "toString", null_toString);

  vm->numClass = AS_CLASS(rewisFindVariable(vm, coreModule, "Num"));
  ...
  vm->stringClass = AS_CLASS(rewisFindVariable(vm, coreModule, "String"));
  ...
  vm->listClass = AS_CLASS(rewisFindVariable(vm, coreModule, "List"));
  ...
  vm->mapClass = AS_CLASS(rewisFindVariable(vm, coreModule, "Map"));
  ...
  vm->rangeClass = AS_CLASS(rewisFindVariable(vm, coreModule, "Range"));
  ...
  ObjClass* systemClass = AS_CLASS(rewisFindVariable(vm, coreModule, "System"));
