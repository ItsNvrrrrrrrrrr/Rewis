...
static uint32_t validateIndexValue(RewisVM* vm, uint32_t count, double value,
                                   const char* argName)
{
  ...
  vm->fiber->error = rewisStringFormat(vm, "$ out of bounds.", argName);
  return UINT32_MAX;
}
...
{
  if (rewisMapIsValidKey(arg)) return true;
