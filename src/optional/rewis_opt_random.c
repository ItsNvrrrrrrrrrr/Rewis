...
#include "rewis_opt_random.rewis.inc"
...
static void randomAllocate(RewisVM* vm)
{
  Well512* well = (Well512*)rewisSetSlotNewForeign(vm, 0, 0, sizeof(Well512));
  well->index = 0;
}

static void randomSeed0(RewisVM* vm)
{
  Well512* well = (Well512*)rewisGetSlotForeign(vm, 0);
...
static void randomSeed1(RewisVM* vm)
{
  Well512* well = (Well512*)rewisGetSlotForeign(vm, 0);

  srand((uint32_t)rewisGetSlotDouble(vm, 1));
  for (int i = 0; i < 16; i++)
  {
    well->state[i] = rand();
  }
...
static void randomSeed16(RewisVM* vm)
{
  Well512* well = (Well512*)rewisGetSlotForeign(vm, 0);

  for (int i = 0; i < 16; i++)
  {
    well->state[i] = (uint32_t)rewisGetSlotDouble(vm, i + 1);
  }
}

static void randomFloat(RewisVM* vm)
{
  Well512* well = (Well512*)rewisGetSlotForeign(vm, 0);
  ...
  rewisSetSlotDouble(vm, 0, result);
}

static void randomInt0(RewisVM* vm)
{
  Well512* well = (Well512*)rewisGetSlotForeign(vm, 0);

  rewisSetSlotDouble(vm, 0, (double)advanceState(well));
}
